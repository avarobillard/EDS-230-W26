...
nsamples <- 300
deviation <- 0.15 # make this a variable!
base_thresh <- 20000
ethresh <- runif(
...
# 300 parameter sets, electricity and year and mean for each- can extract mean and link to parameters
results[[1]]
length(results)
...
# plot - pick one of the 2 parameter as a color, other as x-axis
...
# reverse this- color & x-axis choice
...
# unique identifier for each model run!
...
model output- more important to work on efficiency than ethresh! ethresh has more impact in 1953, etc.
...
# similar to pmap, less efficient
for (i in 1:nsamples) {
  res <- solarpv(
    area = 20, solar = sierraczosolar,
    clr = "green", eunit = "kWhr", g = FALSE, etype = "direct",
    ethresh = parms$ethresh[i], # indexing thru parameters
    eff = parms$eff[i]
  )
...
maybe add a multiplicative factor for price based on year input?
