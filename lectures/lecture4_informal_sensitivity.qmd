...
Declines by 25% between sites AND between parameters- decline in energy is proportional to that parameter
...
For each of 20 parameters, gives mean and each month's value- but need to create data structure for sensitivity analysis -\>
...
# map_dfr(): map data frame by row (when you do the function, create a df)
...
# first come up with varying values for ethresh
# if we don't know ethresh exactly , lets try 20 samples


ethresh <- runif(20, min = 8500, max = 11500)
...
site2 <- ethresh %>% map(~ solarpv(area = 0.1, 
  solar = sierraczosolar, clr = "green", 
  eunit = "kWhr", g = FALSE, etype = "direct", eff = 0.8, ethresh = .x))
head(site2)
```
...
ggplot(site2df, aes(year, elect, group = year)) +
  ...
  labs(y = "Electricity in W") +
  geom_line(data = site2_average, aes(year, elect, group = 1), col = "violet", lwd=2)
```

what matters more, uncertainty in ethresh or eff?
